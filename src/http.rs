...
use crate::effects::{EFFECT_REGISTRY, EffectType};
...
pub fn start_http_server(
    ...
    server.fn_handler::<anyhow::Error, _>("/wifi/scan", esp_idf_svc::http::Method::Get, move |req| {
        match wifi_mgr_scan.scan_networks() {
            Ok(networks) => {
                ...
                let max_networks = networks.len().min(25);  // Giới hạn 25 networks
                for (i, net) in networks.iter().take(max_networks).enumerate() {
                    ...
                    write!(item, "{{\"ssid\":\"{}\",\"rssi\":{},\"auth\":\"{}\"}}",
                        net.ssid, net.rssi, net.auth).unwrap();
    ...
    // POST /wifi/connect - Connect to WiFi
    let wifi_mgr_connect = wifi_manager.clone();
    server.fn_handler::<anyhow::Error, _>("/wifi/connect", esp_idf_svc::http::Method::Post, move |mut req| {
        ...
        if let (Some(ssid), Some(pass)) = (ssid, password) {
            ...
            match wifi_mgr_connect.save_credentials(&ssid, &pass) {  // ← Now using &String
                Ok(_) => {
                    let mut response = req.into_ok_response()?;
            ...
            response.write_all(b"{\"status\":\"error\",\"message\":\"Missing ssid or password\"}")?;
        }
        Ok(())
    })?;
    ...
    // DELETE /wifi/credentials - Clear saved credentials
    let wifi_mgr_clear = wifi_manager.clone();
    server.fn_handler::<anyhow::Error, _>("/wifi/credentials", esp_idf_svc::http::Method::Delete, move |req| {
                ...
                "days" => {
                    ...
                    for day_str in decoded.split(',') {
                        if let Ok(day) = day_str.parse::<u8>() {
                            if day < 7 { days[day as usize] = true; }
                        }
                    }
                }
                "mode" => mode = v,
    ...
    // /schedule/toggle
    let scheduler_clone = scheduler.clone();
    server.fn_handler::<anyhow::Error, _>("/schedule/toggle", esp_idf_svc::http::Method::Post, move |mut req| {
        let mut buf = [0u8; 128];
        let len = req.content_len().unwrap_or(0) as usize;
        let len = len.min(128);
        if len == 0 {
    ...
    server.fn_handler::<anyhow::Error, _>("/status", esp_idf_svc::http::Method::Get, |req| {
        let mut response = req.into_ok_response()?;
        response.write_all(
            b"{\"status\":\"ok\",\"device\":\"ESP32-LED\",\"version\":\"4.0\"}"
        )?;
        Ok(())
    })?;
    ...
    server.fn_handler::<anyhow::Error, _>("/wifi/scan", esp_idf_svc::http::Method::Options, |req| {
        let mut response = req.into_ok_response()?;
        response.write_all(b"")?;
        Ok(())
    })?;
    
    server.fn_handler::<anyhow::Error, _>("/wifi/connect", esp_idf_svc::http::Method::Options, |req| {
        let mut response = req.into_ok_response()?;
        response.write_all(b"")?;
        Ok(())
    })?;
...
fn json_escape(input: &str) -> heapless::String<256> {
    ...
    for c in input.chars() {
        match c {
            ...
            '\\' => { let _ = escaped.push_str("\\\\"); }
            '\u{08}' => { let _ = escaped.push_str("\\b"); }
            '\u{0C}' => { let _ = escaped.push_str("\\f"); }
            '\n' => { let _ = escaped.push_str("\\n"); }
            '\r' => { let _ = escaped.push_str("\\r"); }
            '\t' => { let _ = escaped.push_str("\\t"); }
            _ if c < ' ' || c > '~' => {  // Các ký tự không in được hoặc ngoài ASCII cơ bản
                let _ = write!(escaped, "\\u{:04x}", c as u32);
            }
            _ => { let _ = escaped.push(c); }
        }
    }
    escaped
}
...
fn effect_type_to_str(effect_type: &EffectType) -> &'static str {
    EFFECT_REGISTRY.iter()
        .find_map(|(key, &val)| if val == *effect_type { Some(*key) } else { None })
        .unwrap_or("unknown")
}
